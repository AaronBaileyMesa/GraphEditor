...
struct NodeRenderingTests {
    ...
    @MainActor @Test("Generic node with content displays content text")
    func testGenericNodeWithContent() async {
        let viewModel = createTestViewModel()
        let node = await viewModel.model.addNode(at: CGPoint(x: 50, y: 50))
        
        // Note: Content manipulation APIs vary by node type
    ...
    @MainActor @Test("Selected node renders with yellow stroke")
    ...
    @Test("Font size respects minimum at low zoom")
    func testFontMinimumSize() {
        // NodeView uses: max(8.0, 12.0 * zoomScale) for font size
        // At 0.1 zoom: max(8.0, 1.2) = 8.0
        let zoomScale: CGFloat = 0.1
        let calculatedFontSize: CGFloat = max(8.0, 12.0 * zoomScale)
        #expect(calculatedFontSize == 8.0, "Font should respect minimum of 8.0")
    }
    ...
    func testControlNodeAccessibilityLabel() async {
        ...
        let controlNodes = viewModel.model.nodes.compactMap { $0.unwrapped as? ControlNode }
